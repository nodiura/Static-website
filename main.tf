...
data "aws_route53_zone" "domain_name" {
  name = "nodirabegim.com"  # Must match the Route 53 hosted zone exactly
}
...
resource "aws_route53_record" "route53_record" {
  ...
  zone_id = data.aws_route53_zone.domain_name.zone_id  # Directly reference the zone ID
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  origin {
    ...
    # Define S3 origin configuration
    s3_origin_config {
  ...
  restrictions {
    geo_restriction {
      ...
      locations        = []   # Ensure this value is set accordingly based on requirements
    }
  }
